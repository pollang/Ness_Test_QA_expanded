...
def test_get_current_command_matches_config(ammeter_type, ammeter_cls):
    ...
    expected_command = TEST_AMMETERS[ammeter_type]["command"].encode("utf-8")
...
def test_exact_command_match_returns_data(running_emulators, ammeter_type, _cls):
    port = TEST_AMMETERS[ammeter_type]["port"]
    command = TEST_AMMETERS[ammeter_type]["command"].encode("utf-8")
...
def test_mismatched_command_returns_no_data(running_emulators, ammeter_type, _cls):
    port = TEST_AMMETERS[ammeter_type]["port"]
    wrong_command = TEST_AMMETERS[ammeter_type]["command"].split(" ")[0].encode("utf-8")  # e.g. b"MEASURE_GREENLEE"
