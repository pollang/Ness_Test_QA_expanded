...
**"Easy retrieval and comparison of historical results" (spec section 4)**: `ResultManager.list_results(ammeter_type)` reads every archived run for that ammeter straight off disk — genuinely historical, across sessions, not just what happens to be in memory in the current process. Originally this method (and `compare_results()`) existed but were never actually wired into anything runnable — dead code, found the same way `confidence_interval()` was. Fixed by adding `run_test_suite.py --history`:
...
- `--history --ammeter all --compare` takes the most recent archived run per ammeter and feeds them into the *same* `StatisticsAnalyzer.compare()` used by the live `--compare` path (not the thinner, never-wired `ResultManager.compare_results()`) — so historical and live comparisons produce identically-shaped output, and the result is saved to `results/comparisons/comparison_<timestamp>.json` with `"source": "historical"` so it's distinguishable from a live comparison. This is the actual answer to "identify most reliable measurement method" using archived data instead of requiring a fresh sampling run each time.
- `ResultManager.compare_results(run_ids)` is left in place but still unused by any CLI path — it only returns bare per-run statistics dicts with no CV/CI, which is strictly weaker than routing through `StatisticsAnalyzer.compare()`. Kept as a simple building block, not the primary comparison mechanism.
