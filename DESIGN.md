...
Fixed to pass an `ammeter_type` and to actually start the emulators first (via the new `emulator_launcher.start_emulators`), matching the corrected `AmmeterTestFramework` API — so the code works. The file is marked `# not in use` at the top and not referenced from `README.md`'s usage instructions, matching the original repo's own "don't use it" stance on this file; `run_test_suite.py` is the actual recommended entry point. (If it were ever used, it would need to run as `python -m examples.run_tests` from the repo root, not `python examples/run_tests.py` directly — running it directly puts `examples/` rather than the repo root on `sys.path`, and the `src`/`Ammeters` packages wouldn't resolve, a byproduct of there being no `__init__.py`/packaging in this repo.)
...
## Considered and rejected: capturing each emulator's underlying physics inputs

Each emulator's `measure_current()` computes its reading from randomly-generated inputs (Greenlee: `voltage`/`resistance`; ENTES: `magnetic_field`/`calibration_factor`; CIRCUTOR: a list of 10 `voltages` plus a `time_step`) and prints them to its own console - but only the final `current` float is ever returned from `measure_current()`, sent over the TCP protocol, or captured in `raw_readings`. The intermediate inputs are lost the moment that `print()` call returns.

Considered exposing them (e.g. so a suspicious reading could be traced back to what produced it), but rejected it: this isn't a bug - the emulators work exactly as designed, correctly computing and returning a current value - so there's nothing to "fix." The spec's constraint is to *use* the existing emulator infrastructure and fix only actual errors in it, not extend or redesign it. Capturing these inputs would require either changing `measure_current()`'s contract (currently `-> float` on the abstract base and all three subclasses) or extending the wire protocol's response format beyond a bare current value - both are redesigns of given infrastructure, not error fixes, and out of scope for that reason. Also mechanically harder than it looks: `AmmeterTestFramework` only ever talks to an emulator over its TCP socket (mirroring how a client would talk to real hardware) and never holds a reference to the emulator's Python object, so there's no back door to reach in and read `measure_current()`'s local variables even if we wanted to.
...
- **`AmmeterTestFramework.run_test()` was doing too much in one method** (~40 lines: validate ammeter type, extract config, log, sample, compute stats, build the result record, plot, archive, log again). Split into `_load_run_config()` (validate + extract config sections), `_build_result()` (generate `run_id` + assemble the result dict), and `_maybe_plot()` (the plotting `try/except`), leaving `run_test()` itself as a ~24-line top-to-bottom orchestrator — each step is now one line calling a well-named method.

## `AmmeterTestFramework` couldn't take an injected `ResultManager`/logger

`__init__` always constructed its own `ResultManager` (and originally a `TestLogger`, see below) — no way for a caller to substitute a different implementation without editing `test_framework.py`. Fixed by adding optional constructor parameters, `result_manager` and `logger`, that default to `None` and fall back to today's behavior when not supplied — fully backward compatible, since every existing call site only ever passes `config_path`/`results_dir`.

The `result_manager` type hint stays `Optional[ResultManager]` rather than a formal `Protocol`/ABC — Python's duck typing means any object with the right methods works regardless of what the hint says, and introducing a formal interface for a single real implementation would be premature (there's nothing to abstract over yet). Verified by injecting a plain object (not a `ResultManager` subclass, just matching `ensure_unique_run_id`/`save_result`) and confirming `run_test()` used it correctly and wrote nothing to the real `results/` directory.

## Result became a `TestResult` dataclass, not a plain `Dict`

`run_test()` used to return a bare `dict` — no enforced shape, no typo protection, and the shape was only knowable by reading `_build_result()`'s dict literal. This mattered because it's the framework's most pervasively-used structure: produced by `run_test()`, saved/loaded by `ResultManager`, consumed by `StatisticsAnalyzer.compare()`, printed by all three of `run_test_suite.py`'s reporting functions, and asserted on in tests. Replaced with `TestResult` (and a nested `SamplingTiming`), both plain `@dataclass`es in a new `src/testing/models.py` — `dataclasses` is Python's own standard library, not a new dependency (unlike, say, `pydantic`).

`models.py` exists as its own module (not inside `test_framework.py`) specifically to avoid a circular import: `result_manager.py` needs `TestResult` for its own type hints, and `test_framework.py` already imports `ResultManager` — putting `TestResult` in `test_framework.py` would have made `result_manager.py` import back from it.

**The one real wrinkle, not a reason to avoid it: JSON serialization.** `json.dump`/`json.load` only know how to (de)serialize plain dicts/lists/primitives, not arbitrary class instances — a dataclass needs an explicit conversion step at the JSON boundary, which a plain dict didn't. `TestResult.to_dict()` (`dataclasses.asdict()`, which recurses into the nested `SamplingTiming` automatically) and `TestResult.from_dict()` (reconstructs `SamplingTiming` from the `timing` sub-dict, then the rest of the fields) live on the dataclass itself; `ResultManager.save_result()`/`load_result()`/`list_results()` are the only places that call them — everywhere else in the codebase (the CLI, `StatisticsAnalyzer.compare()`, tests) works with `TestResult` objects directly via attribute access (`result.run_id`, `result.raw_readings`, ...), never a dict. Historical results loaded via `list_results()` go through the same `from_dict()`, so live and historical results are the exact same type throughout — not "class when fresh, dict when loaded from disk."

Verified: the on-disk JSON shape is byte-for-byte the same as before (spot-checked against a fresh run), and `--history`/`--compare` (which specifically exercises loading `TestResult`s back off disk and feeding them into `StatisticsAnalyzer.compare()`) produces identical output to the live path.

## `logger.py`: dropped the `TestLogger` wrapper class for a plain function

`TestLogger` was a thin class wrapping one `logging.Logger`, exposing only 4 passthrough methods (`info`/`error`/`debug`/`warning`) and requiring every caller to instantiate it (`TestLogger("ammeter_test_framework")`) instead of just calling a function. Replaced with `get_logger(context: str = "ammeter_test_framework") -> logging.Logger`, which returns the real `logging.Logger` directly — full API available (`.critical()`, `.exception()`, etc.), no wrapper class to instantiate or maintain. `AmmeterTestFramework.__init__` now does `self.logger = logger or get_logger("ammeter_test_framework")`, and the constructor's `logger` injection parameter is typed `Optional[logging.Logger]`.

Verified this didn't change the actual logging behavior: multiple `AmmeterTestFramework()` instances built in the same process still share one `logging.Logger` object (`logging.getLogger(name)` is a singleton keyed by name) and therefore one log file — confirmed by constructing three instances and checking exactly one file appeared in `results/logs/`, with both instances' log lines landing in it. This is a deliberate, confirmed choice, not an overlooked bug: log files are per-process, not per-run — a separate `python run_test_suite.py` or `pytest` invocation gets its own file (the in-memory logger registry resets between processes), but multiple `run_test()` calls within one process share one file.
